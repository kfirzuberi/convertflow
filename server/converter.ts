...
type Resources = Record<string, ImageBrush>;
type Fonts = Record<string, Buffer>;
...
export async function convertDwfxToPdf(
  inputPath: string,
  outputPath: string,
): Promise<void> {
  ...
  const fixedPage: XmlNode =
    root.FixedPage || Object.values(root)[0];
...
export async function convertDwfxBuffer(
  buffer: Buffer,
  outputPath: string,
): Promise<void> {
...
async function findPages(zip: AdmZip): Promise<PageRef[]> {
  ...
  const fdseqRoot: XmlNode =
    fdseq.FixedDocumentSequence || Object.values(fdseq)[0];
  const pages: PageRef[] = [];
  ...
  for (const child of getChildren(fdseqRoot)) {
    ...
    const docRoot: XmlNode =
      docParsed.FixedDocument || Object.values(docParsed)[0];
...
function readEntry(zip: AdmZip, p: string): string | null {
  const clean = p.replace(/^\.\//, "").replace(/^\//, "");
  const entry = zip.getEntry(clean);
  return entry ? entry.getData().toString("utf-8") : null;
}

function readEntryBuf(zip: AdmZip, p: string): Buffer | null {
  const clean = p.replace(/^\.\//, "").replace(/^\//, "");
  const entry = zip.getEntry(clean);
    ...
    if (child["#name"] === "Canvas.Resources") {
      for (const rd of getChildren(child)) {
        ...
        if (src) {
          ...
          if (rdXml)
            await parseResourceDict(rdXml, basePath, zip, resources);
        }
        for (const res of getChildren(rd))
          loadBrush(res, basePath, zip, resources);
      }
    }
...
async function parseResourceDict(
  xml: string,
  basePath: string,
  zip: AdmZip,
  resources: Resources,
  ...
  const root: XmlNode =
    parsed.ResourceDictionary || Object.values(parsed)[0];
  for (const child of getChildren(root))
    loadBrush(child, basePath, zip, resources);
}
...
function deobfuscateOdttf(buffer: Buffer, fileName: string): Buffer {
...
function drawPathData(doc: PDFKit.PDFDocument, d: string): void {
...
function emitArc(
  ...
  x1: number, y1: number,
  rx: number, ry: number,
  angleDeg: number, largeArc: number, sweep: number,
  x2: number, y2: number,
): void {
...
function bezierArcSeg(
  doc: PDFKit.PDFDocument,
  cx: number, cy: number,
  rx: number, ry: number,
  phi: number, t1: number, t2: number,
): void {
  ...
  const ep = (ct: number, st: number): [number, number] => [
    cosPhi * rx * ct - sinPhi * ry * st + cx,
    sinPhi * rx * ct + cosPhi * ry * st + cy,
  ];

  const [p1x, p1y] = ep(c1, s1);
  const [p2x, p2y] = ep(c2, s2);
  const d1x = -rx * s1, d1y = ry * c1;
  const d2x = -rx * s2, d2y = ry * c2;

  doc.bezierCurveTo(
    p1x + alpha * (cosPhi * d1x - sinPhi * d1y),
    p1y + alpha * (sinPhi * d1x + cosPhi * d1y),
    p2x - alpha * (cosPhi * d2x - sinPhi * d2y),
    p2y - alpha * (sinPhi * d2x + cosPhi * d2y),
    p2x,
    p2y,
  );
}
...
function renderPath(
  ...
  const resMatch = fill?.match(/^\{StaticResource\s+(\w+)\}$/);
  if (resMatch) {
...
function applyStrokeStyle(
  ...
  const cap = (
    node.$.StrokeEndLineCap ||
    node.$.StrokeStartLineCap ||
    "Flat"
  ).toLowerCase();
  doc.lineCap(
  ...
  doc.lineJoin(
    join === "round" ? "round" : join === "bevel" ? "bevel" : "miter",
  );

  doc.miterLimit(parseFloat(node.$.StrokeMiterLimit || "10"));
